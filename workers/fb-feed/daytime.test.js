...
test('parser 8 reprocesses a Thursday source claimed by parser 6 without deleting history',async t=>{
  assert.equal(PARSER_VERSION,8);
  ...
  assert.deepEqual(f.imports().map(r=>r.parser_version),[6,8]);
