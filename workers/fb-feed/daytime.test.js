...
test('parser 5 reprocesses a Thursday source claimed by parser 4 without deleting history',async t=>{
  assert.equal(PARSER_VERSION,5);
  ...
  const oldId=digest(`test:${raw.id}:${captionHash}:${version}:4:${model}`,16);
  f.sql(`INSERT INTO special_imports(id,fb_post_id,fb_created_time,caption_hash,image_source_version,parser_version,model_id,processing_status,fetched_at)
    VALUES(?,?,?,?,?,4,?,'staged',?)`,oldId,raw.id,raw.created_time,captionHash,version,model,raw.created_time);
  await f.run();assert.equal(f.state.aiCalls,1);assert.equal(f.slots(4,'lunch')[0].content,dishes[0]);
  assert.deepEqual(f.imports().map(r=>r.parser_version),[4,5]);
